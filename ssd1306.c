...
char* floatToArray(float number, int decimalPlaces) {
    // Tính độ dài tối đa của chuỗi kết quả
    int maxLength = 3 + 1 + decimalPlaces; // 10 chữ số nguyên + dấu thập phân + số chữ số sau dấu thập phân
    char* result = (char*)malloc(maxLength + 1); // +1 để chứa ký tự null kết thúc chuỗi

    if (result == NULL) {
        // Xử lý lỗi nếu không đủ bộ nhớ
    ...
    // Chuyển đổi phần nguyên thành mảng ký tự
    int i = 0;
    while (integerPart > 0) {
    ...
    // Đảo ngược mảng ký tự vì nó được xây dựng từ phải sang trái
    int start = 0;
    int end = i - 1;
    while (start < end) {
    ...
    // Chuyển đổi phần thập phân thành mảng ký tự
    for (int j = 0; j < decimalPlaces; ++j) {
        decimalPart *= 10;
        result[i++] = '0' + (int)decimalPart;
        decimalPart -= (int)decimalPart;
    }

    result[i] = '\0';  // Kết thúc mảng ký tự bằng ký tự null

    return result;
}


void displayNoti (struct Point_data *located)
{
  SSD1306_ClearScreen (); 

  char* LAT = floatToArray(located->lat, 4);
  char* LONG = floatToArray(located->lng, 4);
  //char* RADIUS = floatToArray(located->radius, 4);

  SSD1306_SetPosition (20, 1);
  SSD1306_DrawString("LOCATION LOCKED");
  ...
  // SSD1306_SetPosition (20, 7);
  // SSD1306_DrawString("radius ");
  // SSD1306_DrawString  (RADIUS);
...
void displayLocate (struct Point_data *pointData)
{
    // char* LAT = floatToArray(pointData->lat, 4);
    // char* LONG = floatToArray(pointData->lng, 4);
    char* LAT = " ";
    char* LONG = " ";
    sprintf(LAT,"%.4f",pointData->lat);
    sprintf(LONG,"%.4f",pointData->lng);
  SSD1306_ClearScreen (); 

    // sprintf(time, "%d / %d / %d ", timeStamp.year, timeStamp. month, timeStamp.day);
    ...
    SSD1306_DrawString(LONG);
        ...
        // sprintf(text, "%d-", pointData->timeStp.year);
        // SSD1306_DrawString(text);
        // sprintf(text, "%d-", pointData->timeStp.month);
        // SSD1306_DrawString(text);
        // sprintf(text, "%d", pointData->timeStp.day);
        // SSD1306_DrawString(text);
        
        SSD1306_SetPosition(45, 7);
        sprintf(text, "%d : ", pointData->timeStp.hour);
        SSD1306_DrawString(text);
        sprintf(text, "%d", pointData->timeStp.minute);
        SSD1306_DrawString(text);
  SSD1306_UpdateScreen (0x3C);
}
/**
